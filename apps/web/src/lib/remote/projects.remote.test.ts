...
const { deleteProject } = await import(
  `./projects.remote?delete-project-test=${Date.now()}`
);
const deleteProjectHandler = deleteProject as unknown as (input: {
  projectId: string;
}) => Promise<{ ok: boolean }>;
...
describe("deleteProject", () => {
  ...
  it("returns success after deleting the project", async () => {
    const result = await deleteProjectHandler({ projectId: "proj-1" });
  ...
  it("throws 404 when the project is not found", async () => {
    ...
    await expect(
      deleteProjectHandler({ projectId: "proj-missing" })
